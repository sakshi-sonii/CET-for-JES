...
import { connectDB, Test, TestSubmission, getUserFromRequest } from "./_db.js";
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      const existingSubmission = await TestSubmission.findOne({
        ...
        studentId: new mongoose.Types.ObjectId(currentUser._id.toString()),
      });
      ...
      for (const section of test.sections) {
        const marksPerQuestion = section.marksPerQuestion || (section.subject === "maths" ? 2 : 1);
        let sectionScore = 0;
        ...
        for (let i = 0; i < section.questions.length; i++) {
          ...
          const studentAnswer = answers[questionKey] !== undefined && answers[questionKey] !== null
            ? Number(answers[questionKey])
            : null;
          const isCorrect = studentAnswer !== null && studentAnswer === question.correct;
